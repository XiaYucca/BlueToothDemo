...
- (void)viewDidLoad {
    ...
    self.tableView.frame = CGRectMake(0, 0, 70, 300);
...
{
    NSLog(@"按下了%ld",indexPath.row);
    
    if ([self.delegate respondsToSelector:@selector(touchViewController:tableView:didSelectRowAtIndexPath:)]) {
        
        [self.delegate touchViewController:self tableView:tableView didSelectRowAtIndexPath:indexPath];
    
    }
    
}

#pragma mark - 自定义放出去的代理方法
